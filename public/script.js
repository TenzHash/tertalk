...
const icebreakers = ["What's the last movie you watched?", "Do you believe in aliens?", "Cats or Dogs?", "What is your dream travel destination?", "If you could have one superpower, what would it be?", "Pizza or Burgers?", "What's the weirdest thing you've ever eaten?", "Coffee or Tea?"];
...
function showModal(title, message, btnText = "OK", onConfirm = null) {
    modalTitle.textContent = title; modalMessage.textContent = message; modalBtn.textContent = btnText;
    customModal.style.display = 'flex';
    modalBtn.onclick = () => { if (onConfirm) onConfirm(); customModal.style.display = 'none'; };
}
...
tabChannel.onmessage = (event) => {
    if (event.data === 'new_tab_opened') {
        socket.disconnect();
        showModal("Connection Paused", "TerTalk is open in another tab.", "Reload Here", () => { window.location.reload(); });
        startBtn.style.display = 'none'; stopBtn.style.display = 'none'; headerStopBtn.style.display = 'none'; toggleInputs(true);
        updateOverlay("Disconnected", "Open in another tab.", false); tabChannel.close();
    }
};
...
window.addEventListener('load', () => {
    ...
    if (savedTheme === 'dark') { document.documentElement.setAttribute('data-theme', 'dark'); themeToggle.textContent = '☀️'; document.querySelector('emoji-picker').classList.add('dark'); }
    if (localStorage.getItem('muted') === 'true') { isMuted = true; muteBtn.textContent = '🔇'; }
});
...
themeToggle.addEventListener('click', () => {
    ...
    if (currentTheme === 'dark') {
        document.documentElement.setAttribute('data-theme', 'light'); localStorage.setItem('theme', 'light'); themeToggle.textContent = '🌙'; picker.classList.remove('dark');
    } else {
        document.documentElement.setAttribute('data-theme', 'dark'); localStorage.setItem('theme', 'dark'); themeToggle.textContent = '☀️'; picker.classList.add('dark');
    }
});
muteBtn.addEventListener('click', () => { isMuted = !isMuted; muteBtn.textContent = isMuted ? '🔇' : '🔊'; localStorage.setItem('muted', isMuted); });
icebreakerBtn.addEventListener('click', () => { if (!isConnected) return; messageInput.value = icebreakers[Math.floor(Math.random() * icebreakers.length)]; messageInput.focus(); });
emojiBtn.addEventListener('click', (e) => { e.stopPropagation(); emojiPopover.style.display = emojiPopover.style.display === 'block' ? 'none' : 'block'; });
...
document.addEventListener('click', (e) => { if (!emojiBtn.contains(e.target) && !emojiPopover.contains(e.target)) emojiPopover.style.display = 'none'; });

cameraBtn.addEventListener('click', () => { if (isConnected) imageInput.click(); });
imageInput.addEventListener('change', () => {
    const file = imageInput.files[0]; if (!file) return;
    if (file.size > 2 * 1024 * 1024) { showToast("Image too large! Max 2MB.", "error"); imageInput.value = ''; return; }
    const reader = new FileReader();
    reader.onload = () => { socket.emit('send_image', reader.result); appendImage(reader.result, 'you', false); };
...
function appendImage(src, sender, isBlurred) {
    ...
    const wrapper = document.createElement('div'); wrapper.classList.add('image-wrapper');
    if (isBlurred) wrapper.classList.add('blurred');
    const img = document.createElement('img'); img.src = src; wrapper.appendChild(img); div.appendChild(wrapper);
    if (isBlurred) wrapper.addEventListener('click', () => { wrapper.classList.remove('blurred'); wrapper.classList.add('revealed'); });
    chatBox.insertBefore(div, typingIndicator); chatBox.scrollTop = chatBox.scrollHeight;
}
function appendMessage(text, sender) { const div = document.createElement('div'); div.classList.add('message', sender); div.textContent = text; chatBox.insertBefore(div, typingIndicator); chatBox.scrollTop = chatBox.scrollHeight; }
function appendSystemMessage(text) { const div = document.createElement('div'); div.classList.add('system-message'); div.textContent = text; chatBox.insertBefore(div, typingIndicator); chatBox.scrollTop = chatBox.scrollHeight; }
function updateOverlay(title, subtitle, showRadar) { statusOverlay.style.display = 'flex'; overlayTitle.textContent = title; overlaySubtitle.textContent = subtitle; radarAnim.style.display = showRadar ? 'block' : 'none'; }
function hideOverlay() { statusOverlay.style.display = 'none'; }
function toggleInputs(disabled) {
    interestsInput.disabled = disabled; nicknameInput.disabled = disabled; ageInput.disabled = disabled; genderInput.disabled = disabled;
...
function sendMessage() {
    ...
    if (msg && isConnected) { socket.emit('send_message', msg); appendMessage(msg, 'you'); messageInput.value = ''; emojiPopover.style.display = 'none'; socket.emit('stop_typing'); }
}
messageInput.addEventListener('input', () => { if (!isConnected) return; socket.emit('typing'); clearTimeout(typingTimeout); typingTimeout = setTimeout(() => { socket.emit('stop_typing'); }, 1000); });
...
socket.on('match_found', (data) => {
    setConnectedState(data.partnerProfile || { nickname: 'Stranger' }); if (!isMuted) matchSound.play().catch(e => {}); showToast("Connected!", "success");
